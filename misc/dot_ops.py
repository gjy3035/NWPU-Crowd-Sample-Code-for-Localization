...
class LocKernel(nn.Module):
    def __init__(self, froze=True):
        super(LocKernel, self).__init__()
        loc_weight = torch.Tensor([[[[0,1,0],
                                     [1,1,1],
                                     [0,1,0]]]])
      
        self.lkernel = nn.Conv2d(1, 1, 3, stride=1, padding=1, bias=False)
        # pdb.set_trace()
        self.lkernel.weight = torch.nn.Parameter(loc_weight)

        if froze: self.frozePara()

    def forward(self, dotmaps):
        gaussianmaps = self.lkernel(dotmaps)
        return gaussianmaps
